...
def send_to_store(cart, aisle_mapping):
    ...
    for item, count in sorted(cart.items(), reverse=True):
        ...
        fulfillment_cart[item] = [count, aisle, refrigerated]
...
def update_store_inventory(fulfillment_cart, store_inventory):
    ...
    """
    for item, infos in fulfillment_cart.items():
        quantity_ordered = infos[0]
        current_stock = store_inventory[item][0]
        new_stock = current_stock - quantity_ordered
        store_inventory[item][0] = new_stock if new_stock > 0 else 'Out of Stock'
